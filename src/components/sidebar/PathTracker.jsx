...
const formatPathPart = (part) => part.charAt(0).toUpperCase() + part.slice(1).replace(/([A-Z])/g, ' $1');;
    ...
    return (
        <div className='px-2 min-[300px]:px-5 flex gap-1 sm:gap-2 text-lg items-center w-full  max-sm:text-sm'>
            ...
            {!isDashboardPage && (
                    <NavLink
                    to="/dashboard"
                    className={({ isActive }) => `${isActive ? "" : "text-sec-color"} trans hover:text-gray-600`}
                    >
                        Dashboard
                    </NavLink>
                ) 
                ...
                return (
                    <div key={index} className="flex items-center gap-1 sm:gap-2">
                        ...
                        {(isLast) ? (
                            ...
                            <span className="text-sec-color">{window.innerWidth < 500 && index > 0 && index < pathParts.length - 1 ? "..." :formatPathPart(decodeURIComponent(part))}</span>
                        )}
                    </div>
                );
            })}
