...
export const getAdmins = createAsyncThunk('admins/getAdmins', async ({page, token}, {rejectWithValue}) => {
    try {
        ...
        if (error.response) {
            ...
            return rejectWithValue(error.response.data || error.response.statusText);
...
export const deleteAdmin = createAsyncThunk(
    ...
    async ({ id, currentUser }, { rejectWithValue}) => {
        try {
            const response = await axios.delete(`https://ecommerce-dot-code.vercel.app/api/user/${id}`, {
                headers
            });
...
export const addNewAdmin = createAsyncThunk(
    ...
    async ({ name, email , password, passwordConfirm}, { rejectWithValue}) => {
        try {
            ...
            const response = await axios.post(`https://ecommerce-dot-code.vercel.app/api/user`, 
                {   
                    name,
                    email,
                    password,
            ...
            if (error.response) {
                const apiError = error.response.data.errors?.[0]?.msg || "Server error occurred";
    ...
    async ({ id, name, email, currentUser}, { rejectWithValue}) => {
        try {
            ...
            if (currentUser._id === id)  { 
                const response = await axios.put(`https://ecommerce-dot-code.vercel.app/api/user/updateMe`, 
                    {
                        name,
                        email,
                        role: "admin",
                ...
                const response = await axios.put(`https://ecommerce-dot-code.vercel.app/api/user/${id}`, 
                    updateData,
                    {
                        headers
                    }
