...
import { spicificSubcategory } from '../../../redux/slices/subCategoryslice'
...
export default function CategoryCard({ product, setProduct }){
  ...
  const [category ,setcategoryid] = useState(subid)
...
const handelbrand = (e)=>{
  setProduct((prev)=>{
    ...
    updatedProduct.Brand = e.target.value || product.Brand
...
const handelSubcategory =(e)=>{
   ...
   <div className='w-full'>
    <select className='w-full px-2 py-1 border border-gray-300 rounded text-black' name='subcategory' id='subcategory' required  onChange={handelSubcategory}>
     {subCategory.length === 0 ? (
       <option disabled className='text-wrap' >
   select category first
       </option>
     ): subCategory.map((subcategory) =>
     <>
     <option  key={subcategory._id}  value={subcategory._id} defaultValue={subCategory._id} >{subcategory.name}</option> 
     </>
     )}
    </select>
   </div>
  ...
  {subCategory.length === 0 ? (
  <option disabled key={subCategory._id}>select category first</option>
) : (
  subCategory.map((item) =>
    item.SubCategoryProducts.map((subcategory) => (
      <option key={subcategory._id} value={subcategory._id || product._id}>
        {subcategory.name }
      </option>
    ))
  )
